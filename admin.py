...
from db_func import add_field, get_all_clients, add_prize, get_prizes, update_prize, delete_prize, link_client_field,\
    get_all_fields, get_info_about_field, update_field, delete_field_db
...
class CreatePrizeForm(FlaskForm):
    name_prize = StringField(validators=[DataRequired()], render_kw={'style': 'width: 300px'})
    file = FileField("Файл", validators=[FileRequired()])
    desc = TextAreaField('Описание', validators=[DataRequired()], render_kw={'style': 'width: 300px'})
...
def admin_func():
    ...
    if request.form.get('choise_client_btn'):
        ...
        if session['size_pole'] != 0:
            if session['id_field'] == 0:
                add_field([session['size_pole'], session['name_prizes'], session['list_coords']])
            else:
                update_field([session['id_field'], session['size_pole'], str(session['name_prizes']),
                              str(session['list_coords']), [], []])
...
def create_field(): # начало создания поля
    ...
    session['id_field'] = 0
    return render_template('create_pole.html', n=0, d=[], flag=False)
...
def create_field_post():
    ...
    if request.form.get('change_pole'): # изменить старое поле
    ...
    elif request.form.get('delete_ship'): # удалить кораблик
        ...
        session['name_prizes'] = session['name_prizes'][:ind] + session['name_prizes'][ind + 1:]
    print(session)
    return render_template('create_pole.html', n=session.get('size_pole'), d=session['list_coords'], flag=flag)
...
def set_ship():
    ...
    if request.form.get('btn'):
        ...
        coords = [int(i) for i in req.split()]
        ind = session['list_coords'].index(coords)
        session['name_prizes'] = session['name_prizes'][:ind] + session['name_prizes'][ind + 1:]
    return render_template('set_prize.html', sl=session['prizes'], coords=f'{coords[0]} {coords[1]}')
